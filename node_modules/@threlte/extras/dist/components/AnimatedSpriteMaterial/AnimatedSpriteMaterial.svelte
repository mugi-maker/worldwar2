<script>import { DoubleSide, FileLoader, LinearFilter, MeshBasicMaterial, NearestFilter, RepeatWrapping, RGBADepthPacking, SpriteMaterial } from 'three';
import { T, asyncWritable, createRawEventDispatcher, useTask, watch, useLoader, useParent } from '@threlte/core';
...
export let fps = 10;
export let filter = 'nearest';
export let alphaTest = 0.1;
export let delay = 0;
export let transparent = true;
export let flipX = false;
export let startFrame = 0;
export let endFrame = undefined;
export let rows = 1;
...
export let is = isMesh ? new MeshBasicMaterial() : new SpriteMaterial();
export let ref;
...
const { start, stop } = useTask(() => {
    ...
    if ((direction === 'forward' && currentFrame > end) ||
...
watch([textureStore, jsonStore], ([nextTexture, nextJson]) => {
    if (nextTexture === undefined || nextJson === undefined)
        return;
