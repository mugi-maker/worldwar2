<script>import { HierarchicalObject, T, useTask, useThrelte } from '@threlte/core';
import { onDestroy, onMount } from 'svelte';
import { CanvasTexture, CapsuleGeometry, Color, Euler, Object3D, OrthographicCamera, Quaternion, Raycaster, Scene, Vector2, Vector3, Vector4 } from 'three';
export let renderTask = undefined;
export let animationTask = undefined;
export let turnRate = 2 * Math.PI;
export let center = [0, 0, 0];
export let verticalPlacement = 'bottom';
export let horizontalPlacement = 'right';
export let size = 128;
export let xColor = 0xff3653;
export let yColor = 0x8adb00;
export let zColor = 0x2c8fff;
export let toneMapped = false;
...
useTask(renderTask?.key ?? Symbol('threlte-extras-gizmo-render'), () => {
    const autoClear = renderer.autoClear;
    renderer.autoClear = false;
    renderer.getViewport(viewport);
    const toneMapping = renderer.toneMapping;
    renderer.toneMapping = toneMapped ? renderer.toneMapping : 0;
    const x = horizontalPlacement === 'left'
        ? paddingX
        : renderer.domElement.offsetWidth - size - paddingX;
    const y = verticalPlacement === 'bottom'
        ? paddingY
...
let p = [0, 0, 0];
useTask(animationTask?.key ?? Symbol('threlte-extras-gizmo-animation'), (delta) => {
...
const getSpriteTexture = (size, colorRepresentation, text = '') => {
